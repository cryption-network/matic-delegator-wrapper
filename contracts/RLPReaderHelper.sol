...
contract RLPReaderContract {
    ...
    function isList(RLPItem memory item) public pure returns (bool) {
        uint8 byte0;
    ...
    function toAddress(RLPItem memory item) public pure returns (address) {
    ...
    function toUint(RLPItem memory item) public pure returns (uint256) {
    ...
    function toUintStrict(RLPItem memory item) public pure returns (uint256) {
        uint256 itemLength = _itemLength(item.memPtr);
    ...
    function toBytes(RLPItem memory item) public pure returns (bytes memory) {
        uint256 listLength = _itemLength(item.memPtr);
    ...
    function toRlpBytesToList(bytes memory b, uint index, uint index2, uint index3, uint index4) public pure returns (bytes memory, bytes memory, uint, address) {
        ...
        RLPItem[] memory rlpList =  toList(rlpItem);
        
        // bytes memory receiptList =  toBytes(rlpList[index]);

        RLPItem[] memory rlpList2 =  toList(rlpList[index]);
        
        
        // for 2nd iteration
        // RLPItem memory rlpItem1 = toRlpItem(rlpList2);
        
        // return toBytes(rlpList2[index2]);
        
        
        // RLPItem[] memory rlpItem3 = toList(rlpList2[index2]);
        
        // bytes memory output1 =  toRlpBytes(rlpList2[index2]);
        
        
        // bytes memory rlpItem3 = toBytes(toList(rlpList2[index2]));
        ...
        RLPItem memory rlpItem = toRlpItem(_test);
